...
public class BookAuthorServerService extends BookAuthorServiceGrpc.BookAuthorServiceImplBase {
    ...
    @Override
    public StreamObserver<Book> getBookByhAuthorGender(StreamObserver<Book> responseObserver) {
        return new StreamObserver<Book>() {
            List<Book> bookList = new ArrayList<>();
            @Override
            public void onNext(Book book) {
                TempDB.getBooksFromTempDb().stream().filter(bookFromDB -> bookFromDB.getAuthorId() == bookFromDB.getAuthorId()).forEach(bookList::add);
            }
